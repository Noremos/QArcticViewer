...
private:
	...
	using skipPrt = unique_ptr<SkipFinderStack>;
    ...
    parcInfo *getInnerSign(string line, bool &multiComment)
    {
        ...
        for (int i = 0; i < line.length(); i++)
        {
            ...
            if (stoken != skipWordParser.end())
            {
                ...
                {
					stokenSecond = skipWordFinder.insert({stoken->second.secondToken, make_unique<SkipFinderStack>()}).first;
                }
...
public:
    ...
    unique_ptr<Uparser> addGetStatementSignature(signature& sig, string line, Uparser* parserList = nullptr)
    {
        ...
        parser->addSingleToken('.', 0, true);
		bool filler;
		parser->processText(line, filler);

        bool mult;
        parcInfo* info = parser->getInnerSign(line, mult);

		sig.init((ps*)info->sign.data(), info->sign.size());
        ...
        for (size_t i = 0; i < sig.size(); i++)
        {
            ...
            if (op == ps::word)
            {
				...
				{
					newSig.push_back(ps::word);
                }
                else
                {
					int wIndex = parser->addWordToken(info->getSub(i));
            ...
            if (op == opWord)
            {
                ...
                {
					int wIndex = parser->addWordToken(info->getSub(i - 1));
                    newSig.push_back(wIndex);
				}
